    ...
    try {
      ...
      return data.map((item: Record<string, unknown>) => {
        ...
        return {
          ...
          author: authorName,
          publishedAt: item.published_at || item.publishedAt ? new Date(item.published_at as string || item.publishedAt as string) : new Date(),
          updatedAt: item.updated_at || item.updatedAt ? new Date(item.updated_at as string || item.updatedAt as string) : new Date(),
          status: item.status,
  ...
  getById: async (id: string): Promise<NewsArticle> => {
    try {
      ...
      return {
        ...
        publishedAt: item.published_at || item.publishedAt ? new Date(item.published_at || item.publishedAt) : new Date(),
